...
固定データによる通常予測のブラウザ計算も試験的に利用できます。最新の気象予測用ではありません。
気象・標高のパッケージ保存・読込に対応しています。操作手順・対応日時と地域は [ブラウザ予測の試験接続](poc/browser-predictor/INTEGRATION.md) を参照してください。

地域GFSパッケージの作成は start-weather-builder.bat から行えます。
[作成手順・高度エラーへの対処](poc/browser-predictor/BUILDER.md) を参照してください。
...
解析モードは、任意のCV・分布を指定する「確率的不確実性解析」、2024～2025年の実績から推定した補正値とばらつきを使う「過去実績検証」、共同実験の固定27条件を確認する「GO基準検証」の3種類です。
