...
function getCurrentLaunchSiteCandidates() {
    var candidates = [];
    var siteMap = window._launch_sites || null;

    $('#site option').each(function () {
        var name = $(this).val();
        if (!name || name === 'Other') return;

        var site = siteMap ? siteMap[name] : null;
        if (!site) return;
...
function runBatchSimulation() {
    ...
    batchNanrekuSitesData = getCurrentLaunchSiteCandidates();
    currentBatchIndex = 0;
...
function plotStandardPrediction(prediction, settings) {
    ...
    // Ocean drift overlay (Leaflet path) - call the common requester if enabled
    try {
        if ($('#enable_drift').length && $('#enable_drift').is(':checked')) {
            console.log('[DEBUG] Triggering ocean drift overlay from pred-new plotStandardPrediction');
            requestOceanDriftOverlay({
                launchLat: launch.latlng.lat,
                launchLon: launch.latlng.lng,
                launchTime: launch.datetime.clone().utc().format(),
                landPoint: { lat: landing.latlng.lat, lng: landing.latlng.lng }
            });
        }
    } catch (_e) {}
...
function loadFishingPortData() {
    ...
    function tryNext(index) {
        if (index >= kmlCandidates.length) {
            appendDebug("漁港データ(KML)の読み込みに失敗しました。");
            var info = document.getElementById('nearest_fishing_ports_info');
            if (info) {
                info.innerHTML = '漁港データを読み込めませんでした。`;frontend/data/漁船・回収地点位置関係マップ.kml` を確認してください。';
            }
            return;
        }

        $.ajax({
            type: "GET", url: kmlCandidates[index], dataType: "xml",
