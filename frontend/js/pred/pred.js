...
function readURLParams() {
    ...
    if(url.searchParams.has('enable_drift')){
        var driftEnabled = url.searchParams.get('enable_drift');
        $("#enable_drift").prop('checked', driftEnabled !== 'false' && driftEnabled !== '0');
    }
    if(url.searchParams.has('drift_hours')){
        $("#drift_hours").val(url.searchParams.get('drift_hours'));
    }
...
function changeLaunchSite() {
    ...
    if ( selectedName == "Other" ) {
        ...
        // もしキャッシュがあれば同期的に反映する（UI操作で即時反映されない問題の対策）
        var sites = window._launch_sites;
        if (sites) {
            $.each(sites, function(sitename, site) {
        ...
        // フォールバック: 非同期で取得してからプロット
        $.getJSON("sites.json", function(sites2) {
            $.each(sites2, function(sitename, site) {
               if ( selectedName == sitename ) {
                    $("#lat").val(site.latitude);
                    $("#lon").val(site.longitude);
                    $("#initial_alt").val(site.altitude);
...
function parseCSV(lines) {
    ...
    if ($('#enable_drift').length && $('#enable_drift').is(':checked')) {
        requestOceanDriftOverlay({
            launchLat: parseFloat(launch_lat),
            launchLon: parseFloat(launch_lon),
...
function getOceanDriftApiBase() {
    if (typeof window.PREDICTOR_API_BASE === 'string' && window.PREDICTOR_API_BASE.trim() !== '') {
        return window.PREDICTOR_API_BASE.replace(/\/$/, '');
    }
    return '';
}

function requestOceanDriftOverlay(context) {
    if (!context || !isFinite(context.launchLat) || !isFinite(context.launchLon)) {
        return;
    }

    var driftInput = document.getElementById('drift_hours');
    var driftHours = driftInput ? parseInt(driftInput.value, 10) : 6;
    if (!isFinite(driftHours) || driftHours < 1) {
        driftHours = 6;
    }
    ...
    var hour = $('#hour').val();
    var minute = $('#min').val();
    var launchMoment = moment.utc([parseInt(year, 10), parseInt(month, 10) - 1, parseInt(day, 10), parseInt(hour, 10), parseInt(minute, 10), 0, 0]);
    var launchIso = launchMoment.format();

    var ascent = $('#ascent').val();
    var burst = $('#burst').val();
    var drag = $('#drag').val();

    var baseUrl = getOceanDriftApiBase();
    var apiEndpoint = (baseUrl ? baseUrl : '') + '/api/simulate';
    var dataParams = {
        lat: context.launchLat,
        lon: context.launchLon,
        time: launchIso,
        ascent_rate: parseFloat(ascent) || 5.0,
        burst_alt: parseFloat(burst) || 30000.0,
        descent_rate: parseFloat(drag) || 10.0,
        hours: parseInt(driftHours, 10) || 6,
        ocean_drift: true
    };
    ...
    appendDebug('Requesting ocean drift overlay...');
    ...
    $.ajax({
        ...
        success: function (csvText) {
            try {
                if (typeof setOceanDriftStatus === 'function') {
                    setOceanDriftStatus('海流予測完了', 'success');
                }
                plotOceanDriftOverlay(csvText, context.landPoint);
                appendDebug('Ocean drift overlay added.');
                setTimeout(function () {
                    if (typeof clearOceanDriftStatus === 'function') {
                        clearOceanDriftStatus();
                    }
                }, 900);
            } catch (err) {
                if (typeof setOceanDriftStatus === 'function') {
                    setOceanDriftStatus('海流予測の表示に失敗', 'error');
                }
                appendDebug('Ocean drift overlay failed: ' + err.message);
                console.error(err);
            }
        },
        error: function (xhr, status, error) {
            var msg = 'Ocean drift overlay request failed: ' + (error || status) + ' (status=' + xhr.status + ')';
            if (typeof setOceanDriftStatus === 'function') {
                setOceanDriftStatus('海流予測に失敗', 'error');
                setTimeout(function () {
                    if (typeof clearOceanDriftStatus === 'function') {
                        clearOceanDriftStatus();
                    }
            ...
            appendDebug(msg);
            try { console.error('[DEBUG] Response body:', xhr.responseText); } catch (_e) {}
        }
    });
}

function plotOceanDriftOverlay(csvText, landPoint) {
    if (!csvText) {
        return false;
    }
    ...
    if (map_items['splash_marker']) {
        ...
        delete map_items['splash_marker'];
    }

    var drift_polyline = L.polyline(driftPath, {
        color: '#2563eb',
        weight: 4,
        opacity: 0.85,
        dashArray: '6, 8'
    ...
    var splash_marker = L.circleMarker(driftPath[0], {
        radius: 6,
        color: '#0f172a',
        weight: 2,
        fillColor: '#38bdf8',
    ...
    if (typeof map.fitBounds === 'function') {
        ...
        if (map_items['path_polyline'] && map_items['path_polyline'].getBounds) {
            bounds.extend(map_items['path_polyline'].getBounds());
        }
