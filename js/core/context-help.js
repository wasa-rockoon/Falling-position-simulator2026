    ...
    var CONTENT = {
        ...
        uncertainty: {
            ...
            sections: [
                ...
                ['計算回数と判定間隔', '最低回数は早期終了を認める最低計算数、最大回数は地点ごとの上限です。判定間隔8なら8、16、24…回で確認します。同時通信数ではありません。最低12・間隔8では最速の早期終了は24回です。'],
                ['海上率と95%信頼区間', '海陸判定できた予測のうち、海に落ちた割合が海上率です。例えば40回中32回なら80%。ただし別のサンプルで計算すれば割合は変わります。その有限回数による推定の幅が信頼区間です。95%は海に落ちる確率ではなく、同じ手順を何度も繰り返したときに作った区間がモデル上の海上確率を含む割合の目安です。気象予報やモデル自体の誤差まで含むものではありません。'],
                ...
                ['安定性による早期終了', '平均着地点の移動許容値1 kmは前回集計から平均位置が動く距離で、着地範囲の半径ではありません。推定幅と平均位置の条件が2回連続で成立し、最低回数以上かつ海陸判定済み80%以上なら早期終了します。楕円や等高線の形は停止判定に使いません。通常の95%信頼区間を繰り返し確認して停止するため、早期終了後の区間に厳密な95%の被覆を保証する仕組みではありません。'],
                ...
                ['通信上限', '全地点のHTTP試行数の上限で再試行を含みます。キャッシュ利用は通信を消費しません。事前計画は上限を地点数で均等分割します。上限100・最大48なら1地点48回、2地点各48回、3地点各33回です。再試行があると早く停止する場合があります。'],
