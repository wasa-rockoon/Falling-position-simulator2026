(function (root) {
    ...
    var CONTENT = {
        autoSearch: {
            title: '放球自動探索',
            sections: [
                ['この機能でできること', '複数の日時範囲・放球地点・飛行条件を組み合わせて自動予測し、雨・風・海率などの条件を満たす候補を探します。例えば9/16 7:00～15:00と9/17 7:00～15:00を一度に指定できます。'],
                ['入力項目の意味', '「日時範囲を追加」で離れた期間を追加できます。飛行条件は現在のSETTINGSと保存済みプリセットから複数選択でき、上昇速度・下降速度・破裂高度だけを使用します。海率下限は「この値以上なら候補に残す」という下限です。高速は粗探索で候補を絞り、全候補精密は粗探索で不合格にせず全候補を精密計算します。段階的は有望な候補から計算します。API呼び出し上限は再試行を含むHTTP試行数の上限です。'],
                ['実行手順', '日時範囲、間隔、飛行条件、探索モード、対象地点を設定します。全日時×全地点×全飛行条件が候補になります。表示される呼び出し回数と所要時間を確認して開始してください。「新規探索」では直前の入力を引き継ぎます。中断は現在のAPI呼び出しが終わったところで次へ進まず停止し、「中止して新規探索」は進行中の通信を中止します。'],
